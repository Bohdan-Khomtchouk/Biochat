...
Similarly to the concept behind the Proof-of-Work (PoW) algorithm, the ranking of output results will only be affected system-wide once consensus is reached amongst a sufficiently diverse set of users.  This is similar to the concept of cryptocurrency miners competing against each other to complete transactions on the network and getting rewarded for it with Bitcoin.  As such, for any given input, users are motivated/incentivized to click on PMID outputs that will match the double-blind activity of other geographical users.  Instead of being offered a coin as a reward for these "mining" efforts, users are rewarded with a re-ordered set of output results that more accurately reflects current community consensus via the collective intelligence that was captured during this process.  As such, `Biochat` is driven by altruism not greed, since it would technically be possible to call up your (X number) of buddies throughout the world and have everybody click on the same output for some identical input search string.  Although such a comical scenario cannot be ruled out when the extrinsic motivator is purely financial (e.g., a new altcoin), it can certainly be ruled out when the intrinsic motivator is altruistic in nature -- benefiting biological data science through your set of skills, knowledge, and expertise in a given area(s) of inquiry.             
